...
function init_github() {
  ...
  git remote add origin-github "https://x-access-token:${token}@github.com/chewiebug/gcviewer.git" > /dev/null 2>&1 || true
}
...
function perform_release() {
  ...
  if [ "${DRY_RUN}" = "true" ]; then
    ...
    # needed for commits being done by the maven release plugin
    init_github
